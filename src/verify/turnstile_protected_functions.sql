...
BEGIN
  ...
  IF obj_description('vibetype.email_address_verification_request(text, text, text)'::regprocedure, 'pg_proc') NOT LIKE '%@turnstileProtected%' THEN
    RAISE EXCEPTION 'vibetype.email_address_verification_request is missing the @turnstileProtected tag';
  END IF;
END $$;
