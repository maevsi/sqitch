...
BEGIN
  ...
  IF NOT (SELECT pg_catalog.has_function_privilege('vibetype_account', 'vibetype_private.event_search_rank(TEXT)', 'EXECUTE')) THEN
    RAISE EXCEPTION 'Test privileges failed: vibetype_account does not have EXECUTE privilege on event_search_rank';
  END IF;

  IF NOT (SELECT pg_catalog.has_function_privilege('vibetype_anonymous', 'vibetype_private.event_search_rank(TEXT)', 'EXECUTE')) THEN
