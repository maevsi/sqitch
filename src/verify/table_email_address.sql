...
SELECT id, subject_id, address, address_hash, status, status_reason, created_at, updated_at, updated_by
...
BEGIN
  ...
  ASSERT (SELECT pg_catalog.has_schema_privilege(current_setting('role.vibetype_username'), 'vibetype_private', 'USAGE'));
END $$;
