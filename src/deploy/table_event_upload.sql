...
COMMENT ON COLUMN vibetype.event_upload.id IS E'@behavior -insert -update\nPrimary key, uniquely identifies each event-upload association.';
COMMENT ON COLUMN vibetype.event_upload.event_id IS E'@behavior -update\nReference to the event associated with the upload.';
...
COMMENT ON COLUMN vibetype.event_upload.upload_id IS E'@behavior -update\nReference to the uploaded file.';
...
CREATE POLICY event_upload_insert ON vibetype.event_upload FOR INSERT WITH CHECK (
  ...
  AND
  EXISTS (
    SELECT 1 FROM vibetype.upload u
    WHERE u.id = event_upload.upload_id
      AND u.created_by = vibetype.invoker_account_id()
  )
);
