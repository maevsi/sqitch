...
-- Lives in the public vibetype schema (not vibetype_private) so vibetype_account/vibetype_anonymous can call it with a plain EXECUTE grant, without needing USAGE on vibetype_private, which would make every other vibetype_private function already EXECUTE-granted to them (e.g. account_block_ids(), used in RLS policies) directly callable too.
...
-- Only vibetype_account/vibetype_anonymous need EXECUTE on this function itself; tsquery_prefix() above needs no direct grant to them since it is now only ever called from here, running as this function's owner.
