...
BEGIN
  ...
  SELECT upload_storage_key FROM maevsi.upload INTO _eventAuthorProfilePictureUploadStorageKey WHERE upload.account_id = _eventAuthorProfilePictureAccountId;
...
CREATE FUNCTION maevsi.profile_picture_set(upload_id uuid DEFAULT gen_random_uuid()) RETURNS void
...
BEGIN
  IF (COALESCE((
    ...
    FROM maevsi.upload
    WHERE account_id = current_setting('jwt.claims.account_id', true)::UUID
  ), 0) + $1 <= (
...
CREATE POLICY contact_delete ON maevsi.contact FOR DELETE USING (((author_account_id = (current_setting('jwt.claims.account_id'::text, true))::uuid) AND (account_id <> (current_setting('jwt.claims.account_id'::text, true))::uuid)));
